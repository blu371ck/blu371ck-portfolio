...
const Navigation = () => {
    ...
    useEffect(() => {
        ...
        if (isMobileMenuOpen) {
            const firstFocusableElement = mobileNavRef.current?.querySelector('a');
    ...
    return (
        <>
            ...
            <header className="md:hidden fixed top-0 left-0 w-full bg-black/50 backdrop-blur-sm h-16 z-40 flex items-center justify-between px-4">
                 ...
                 <button ref={mobileNavOpenButtonRef} onClick={() => setMobileMenuOpen(!isMobileMenuOpen)} className="text-htb-500 z-50" aria-label={isMobileMenuOpen ? "Close mobile menu" : "Open mobile menu"} aria-expanded={isMobileMenuOpen} aria-controls="mobile-nav-panel">
                     <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16m-7 6h7" /></svg>
                 </button>
            </header>
            
            <div data-testid="mobile-nav" id="mobile-nav-panel" ref={mobileNavRef} className={`md:hidden fixed top-0 left-0 w-full h-full bg-gray-900 z-30 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`} role="dialog" aria-modal="true" aria-label="Main menu">
