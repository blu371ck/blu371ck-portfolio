...
const Navigation = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <>
      ...
      <button onClick={() => setSideNavOpen(!isSideNavOpen)} className={`hidden md:flex fixed top-1/2 -translate-y-1/2 h-10 w-10 items-center justify-center rounded-full focus:outline-none ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white transition-all duration-300 ease-in-out z-30 ${isSideNavOpen ? 'left-52' : 'left-5'}`} aria-label={isSideNavOpen ? 'Close navigation' : 'Open navigation'} title={isSideNavOpen ? 'Close navigation' : 'Open navigation'}>
